BLACKBAUD.netcommunity.CartDisplay = new function () {
	...
	function ChargeTransaction(data, sCallbackURI, successCallback, errroCallback) {	
        // Disable the main form		   

		$.ajax({
			url: BLACKBAUD.netcommunity.WebMethodsURL + "/ChargeTransaction",
			type: "POST",
	...
	function GetPublicKey() {
		let data = { resetPublicKey: false }
					, PublicKeyID;
		$.ajax({
			url: BLACKBAUD.netcommunity.WebMethodsURL + "/GetPublicKey",
			type: "POST",
			data: JSON.stringify(data),
			contentType: "application/json; charset=utf-8",
			dataType: "json",
	...
	this.completeCallback = function (e) {		
		if (e.originalEvent !== undefined) {
			...
			if (!that.isPaymentDone) {
				...
				ChargeTransaction(chargeTransdata, that.sCallbackURI, this.ChargeTransactionSuccess, this.ChargeTransactionError);
				that.isPaymentDone = true;
			}
		}
	};

	this.MakePayment = function (dCartTotal, sCallbackURI, oMerchantAccountBBPSID, oMerchantAccountISO4217, cardHolder, addInfo, description, useMasterpass, useVisaCheckout, useApplePay, hasProcessCard, hasStoreCard, cardToken) {
